<div class="fixed z-30 inset-0 overflow-y-auto" x-show="isOpenModalModifyType"
	@edit-type.window="openModalModifyType($event.detail.url, $event.detail.title, $event.detail.description)" x-cloak>

	<div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">

		<div x-show="isOpenModalModifyType" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0"
		...
		<div x-show="isOpenModalModifyType" x-transition:enter="ease-out duration-300"
			x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
			x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200"
			x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
			x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" @click.away="closeModalModifyType"
			@keydown.escape="closeModalModifyType"
				...
				<div class="sm:flex sm:items-start">
					...
					<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
						<div class="flex items-center justify-between">
							<h3 class="text-lg leading-6 font-medium text-gray-700">Créer un nouvau type</h3>
						</div>

						<div class="mt-5">
							<form action="" method="POST" class="w-full" id="modifyTypeForm">
								@csrf
								<div>
									<div class="rounded-md shadow-sm">
										<input type="text" name="title" id="" placeholder="Chambre VIP"
								...
								<div class="mt-3">
									<div class="rounded-md shadow-sm">
										<input type="text" name="description" id="" placeholder="Safari et visite guidée pour les vaccances"
											class="appearance-none block w-full px-3 py-2 border border-gray-300 font-medium rounded-md placeholder-gray-400 focus:outline-none focus:border-blue-500 transition duration-150 ease-in-out sm:text-sm sm:leading-5">
									</div>
								</div>
							</form>
			...
			<div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
				<button type="submit" @click.prevent="modifyType" id="modifyTypeAction"
				...
				<button type="button" @click.prevent="closeModalModifyType"
